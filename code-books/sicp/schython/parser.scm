    ...
    (let ((char (peek-char)))
      (cond ((char-newline? char)
	    ...
	    ((eof-object? char)
		 ...
		 '()))
	    ((eq? char #\space) (read-char) (get-tokens braces))
	    ((eq? char #\#) (ignore-comment) '())
	    ((memq char (list #\[ #\( #\{))
	     (cons (char->symbol (read-char)) (get-tokens (cons char braces))))
	    ((memq char (list #\] #\) #\}))
	     (if (and (not (null? braces)) (eq? char (reverse-brace (car braces))))
		 (cons (char->symbol (read-char)) (get-tokens (cdr braces)))
		 (read-error "SyntaxError: mismatched brace: " char)))
	    ((memq char (list #\, #\:))
	     (cons (char->symbol (read-char)) (get-tokens braces)))
	    ((memq char (list #\" #\'))
	     (cons (list->string (get-string (read-char)))
		   (get-tokens braces)))
	    ((memq char operators) (cons (get-operator) (get-tokens braces)))
	    ((char-numeric? char)
	     (let ((num (get-num "")))
	       (if (string? num)
